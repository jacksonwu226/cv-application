...
export default function Experience({experienceList, setExperienceList}){
  ...
  return(
    <div className="experience-section-container">
      <button className="experience-section-trigger" onClick={() => setSectionOpen(!sectionOpen)}>
        <h2>Experience</h2>
        <img src={caretIcon} className={`caret ${sectionOpen ? "active" : ""}`} />
      </button>
      {showExperienceList ? (
        <ExperienceList 
          experienceList={experienceList} updateSelectedItem={updateSelectedItem}
          appendExpList={appendExpList} addNewExpItemDisplay={addNewExpItemDisplay} removeExpItem={removeExpItem}
          sectionOpen={sectionOpen}
        />
      ) : (
        <ExperienceForm 
        selectedExpItem={selectedExpItem} updateExpList={updateExpList} cancelUpdateExp={cancelUpdateExp} sectionOpen={sectionOpen}
        />
      )}
    </div>
  )
...
function ExperienceList({experienceList, updateSelectedItem, addNewExpItemDisplay, removeExpItem, sectionOpen}){
  return(
    <div className={`experience-list-container ${sectionOpen? "active" : ""}`}>
      {
        experienceList.map((item) => {
          return <ExperienceListItem key={item.id} item={item} updateSelectedItem={updateSelectedItem} removeExpItem={removeExpItem} />
        })
      }
      <div className="add-new-experience-btn">
        <button type='button' onClick={addNewExpItemDisplay}>Add New</button>
      </div>
    </div>
  )
}
...
function ExperienceForm({
  selectedExpItem, updateExpList, cancelUpdateExp, sectionOpen
}) {
  function handleSubmit(event) {
    ...
    const newExpItem = new ExperienceItem(company, position, startDate, endDate, location, description); // Include description in newExpItem
    ...
    <div className={`experience-info-container ${sectionOpen ? "active" : ""}`}>
      <form onSubmit={handleSubmit} className="experience-info-form">
        ...
        <input
          ...
          name="position"
          id="position"
          defaultValue={selectedExpItem === null ? '' : selectedExpItem.position}
        />
        ...
        <input
          ...
          name="start-date"
          id="start-date"
          defaultValue={selectedExpItem === null ? '' : selectedExpItem.startDate}
        />
        ...
        <input
          ...
          name="location"
          id="location"
          defaultValue={selectedExpItem === null ? '' : selectedExpItem.location}
        />
        ...
        <textarea
          id="description"
          name="description"
          defaultValue={selectedExpItem === null ? '' : selectedExpItem.description} // Added defaultValue for description field
        />
